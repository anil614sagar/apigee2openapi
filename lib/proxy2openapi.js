              ...
              loadXMLDoc(location + "/apiproxy/policies/" + flowStepPath.Name + ".xml", function(err, replyStep) {
                ...
                if (replyStep.ExtractVariables) {
                  ...
                  if (source == 'request') {
                    ...
                    parameterObj.in = 'header'
                    addParamsFromExtractVariables(replyStep.ExtractVariables.Header, parameterObj, openapiJson.paths[resourcePath][resourceVerb])
                    // Capture QueryParam
                    parameterObj.in = 'query'
                    addParamsFromExtractVariables(replyStep.ExtractVariables.QueryParam, parameterObj, openapiJson.paths[resourcePath][resourceVerb])
                    // Capture FormParam
                    parameterObj.in = 'formData'
                    addParamsFromExtractVariables(replyStep.ExtractVariables.FormParam, parameterObj, openapiJson.paths[resourcePath][resourceVerb])
                  }
                }
              })
            }
...
function addParamsFromExtractVariables(paramArr, parameterObj, openapiJson) {
  for (key in paramArr) {
    var path = JSON.parse(JSON.stringify(paramArr[key]));
    parameterObj.name = path.$.name
