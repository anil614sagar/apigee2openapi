...
var proxy = require('./proxy2swagger.js');
      ...
      apiBundle.downloadProxy(answers, function(err) {
        ...
        stream.on('close', function(){
          if (!had_error) {
            ...
            // Generate multiple swagger files based on number of files in proxies.
            ...
            glob(destination + "/apiproxy/proxies"+ "/*.xml", options, function (er, files) {
              async.each(Object.keys(files), function (i, callback) {
                proxy.genSwagger(destination, answers, files[i], function (err, reply) {
                  if (err) {
                    callback(err, {});
                  }
                  callback(null, {});
