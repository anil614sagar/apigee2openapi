...
function genSwagger(location, answers, xmlFile, cb) {
  var swaggerJson = {};
  loadXMLDoc(location + "/apiproxy/" + answers.api + ".xml", function(err, reply) {
    ...
    loadXMLDoc(xmlFile, function(err, replyProxy) {
      ...
      fs.writeFile(location + "/"+ jsonFileName  +".json", JSON.stringify(swaggerJson, null, 2), function(err) {
        if(err) {
          cb(err, {});
        }
        console.log("Swagger JSON File successfully generated in : " + location + "/"+ jsonFileName  +".json");
        cb(null, {});
      });
    });
  });
